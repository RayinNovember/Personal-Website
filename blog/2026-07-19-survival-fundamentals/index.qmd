...
where $T$ is the event time, $t_0$ is the time already survived, and $T-t_0$ is the remaining time. The expected remaining lifetime can be calculated if the survival function is known, or estimated based on simulations. The latter provides more information about the distribution of remaining lifetime, which can be used to calculate percentiles rather than a single expected value.
