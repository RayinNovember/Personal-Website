...
### Terminal Nodd CHF Estimation and Ensemble Averaging
...
With the CPH model, the linear combination of covariates - $\eta=\beta_1X_1+\beta_2X_2+...+\beta_pX_3$ represents the log hazard ratio. Each leaf output represents an increment from the current ensemble's log hazard ratio for any subject landing in that leaf.
...
where $\eta_{ij}=\beta_1X_{1i}+\beta_2X_{2i}+...+\beta_pX_{3i}$ is the linear combination of covariates for subject i;
...
With the AFT model, the log-time is being modeled as the reponse of a linear regression:
...
By the chain rule, the graident is:
...
In Python, the random survival forest algorithm is implements through RandomSurvivalForest of the sksurv package (`sksurv.ensemble'). The standard xgb function from the xgboost package can be used to implement the CPH and AFT model with their parameters set up accordingly.
